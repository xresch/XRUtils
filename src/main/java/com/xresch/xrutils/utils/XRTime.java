...
public class XRTime {
	...
	private static DateTimeFormatter formatterTimestamp = DateTimeFormatter.ofPattern(XRTime.FORMAT_TIMESTAMP).withZone(machineTimezone.toZoneId());
	private static DateTimeFormatter formatterISODate = DateTimeFormatter.ofPattern(XRTime.FORMAT_ISO8601_DATE).withZone(machineTimezone.toZoneId());
	
	public static final int SECONDS_OF_1MIN = 60;
	public static final int SECONDS_OF_5MIN = 60 * 5;
	public static final int SECONDS_OF_10MIN = 60 * 10;
	public static final int SECONDS_OF_15MIN = 60 * 15;
	public static final int SECONDS_OF_60MIN = 60 * 60;
	...
	public static int getMachineTimeZoneOffSetMinutes(){
		int offsetMillis = XRTime.getMachineTimeZone().getOffset(System.currentTimeMillis());

		return (int) ( -1 * XRTimeUnit.m.convert(offsetMillis) );
	...
	public static String currentTimestamp(){
		...
		return XRTime.formatDateAsTimestamp(ZonedDateTime.now());
	}
	...
	public static Timestamp getDefaultAgeOutTime(int granularityMinutes) {
		...
		if		(granularityMinutes <= 3) 		{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, 0, 0, -30); }
		else if (granularityMinutes <= 15) 		{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, 0, -1, 0); }
		else if (granularityMinutes <= 60) 		{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, -1, 0, 0); }
		else if (granularityMinutes <= 240) 	{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, -7, 0, 0); }
		else if (granularityMinutes <= 720) 	{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, -14, 0, 0); }
		else if (granularityMinutes <= 1440) 	{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, -30, 0, 0); }
		else  									{ ageOutOffset = XRTime.getCurrentTimestampWithOffset(0, 0, -90, 0, 0); }
